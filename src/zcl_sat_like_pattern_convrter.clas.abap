...
CLASS zcl_sat_like_pattern_convrter DEFINITION
  ...
  PRIVATE SECTION.
    CONSTANTS c_sap_any_single TYPE c LENGTH 1 VALUE '+'.
    CONSTANTS c_sap_any_sequence TYPE c LENGTH 1 VALUE '*'.

    CONSTANTS c_sap_escape TYPE c LENGTH 1 VALUE '#'.

    CONSTANTS c_sql_any_single TYPE c LENGTH 1 VALUE '_'.
    CONSTANTS c_sql_any_sequence TYPE c LENGTH 1 VALUE '%'.
...
CLASS zcl_sat_like_pattern_convrter IMPLEMENTATION.
  ...
  METHOD conv_sap_to_sql_pattern.
    ...
    DATA lv_input_maxpos TYPE i.
    DATA lv_curr_pos TYPE i VALUE 0.
    DATA lv_continue_off TYPE i.
    DATA lv_charlen TYPE i.

    CLEAR ev_sql_pattern.

    lv_input_maxpos = strlen( iv_sap_pattern ).
    lv_input_maxpos = lv_input_maxpos - 1.
    ...
    WHILE lv_curr_pos <= lv_last_pos.
      ...
      CASE iv_sap_pattern+lv_curr_pos(1).
        ...
        WHEN c_sap_escape.
          DATA next_char TYPE c LENGTH 1.
  ...
  METHOD conv_sql_to_sap_pattern.
    ...
    DATA lv_next_char TYPE c LENGTH 1.
    DATA lv_next_char_pos TYPE i.
    DATA lv_next_char_len TYPE i.
