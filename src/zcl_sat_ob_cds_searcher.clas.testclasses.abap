...
CLASS ltcl_abap_unit DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA mr_cut TYPE REF TO zcl_sat_ob_cds_searcher.
...
CLASS ltcl_abap_unit IMPLEMENTATION.
  ...
  METHOD test_no_query.
    TRY.
        mr_cut = NEW #( ir_query = zcl_sat_object_search_query=>parse_query_string( iv_query = '' iv_search_type = zif_sat_c_object_browser_mode=>cds_view ) ).
  ...
  METHOD test_negation_query.
    TRY.
        mr_cut = NEW #(
            ir_query = zcl_sat_object_search_query=>parse_query_string(
