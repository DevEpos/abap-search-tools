...
define view ZSAT_I_CdsFromPartEntity
  as select from dd26s as BaseTable
    ...
    inner join   dd02l as DbTable
      on  BaseTable.tabname = DbTable.tabname
...
union
  select from  dd26s as BaseTable

    inner join tadir as Repo
      on  BaseTable.tabname = Repo.obj_name
...
union
  select from  dd26s                as BaseTable
    ...
    inner join ZSAT_I_DdlDependency as DdlMap
      on BaseTable.tabname = DdlMap.ViewName
...
union
  select from ZSAT_I_CdsBaseTable as BaseTable
...
where BaseTable.TadirType = 'STOB'

-- Base entities of CDS View entities
union
  select from zsatcds2mbtab
