...
CLASS zcl_sat_ob_dbtab_searcher DEFINITION
  PUBLIC
  CREATE PUBLIC
  ...
  PRIVATE SECTION.
    ...
    METHODS add_search_terms_to_condition.
ENDCLASS.



CLASS zcl_sat_ob_dbtab_searcher IMPLEMENTATION.
...
***    CHECK mf_excluding_found = abap_false.
  ...
  METHOD add_search_terms_to_condition.
    DEFINE _add_filter.
    ...
    LOOP AT mo_search_query->mt_search_term ASSIGNING FIELD-SYMBOL(<ls_term>).
      _add_filter <ls_term>.
      CHECK sy-tabix <> lines( mo_search_query->mt_search_term ).
      IF lf_and = abap_true.
        new_and_cond_list( ).
      ELSE.
        new_or_cond_list( ).
