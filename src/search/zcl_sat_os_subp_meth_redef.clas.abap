...
CLASS zcl_sat_os_subp_meth_redef IMPLEMENTATION.
  ...
  METHOD prepare_search.
    set_base_select_table( iv_entity = zif_sat_c_select_source_id=>zsat_i_redefinedmethod
    ...
    add_filter( VALUE #( field  = |{ c_alias_names-method }~{ c_fields-isabstract }|
                         sign   = 'I'
                         option = 'EQ'
                         low    = abap_false ) ).
  ...
  METHOD add_method_name_filter.
    ...
    add_search_terms_to_search(
        it_search_terms = VALUE #( BASE lr_method_names->*
