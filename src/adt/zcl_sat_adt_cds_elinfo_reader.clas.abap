"! <p class="shorttext synchronized">Reads element information for a CDS view</p>
CLASS zcl_sat_adt_cds_elinfo_reader DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Create ADT Object reference for AMDP class</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Retrieve association of cds view (with ddlnames)</p>
    ...
    "! <p class="shorttext synchronized">Retrieve API state and text for CDS view</p>
    ...
    "! <p class="shorttext synchronized">Fills Base table element information</p>
    METHODS fill_base_table_infos
      IMPORTING
        it_base_tables TYPE zsat_cds_view_base_table_t.

    "! <p class="shorttext synchronized">Fills association element information </p>
    METHODS fill_association_infos
      IMPORTING
        it_association TYPE ty_t_association.

    "! <p class="shorttext synchronized">Read additional request parameters</p>
    METHODS get_parameters
      RAISING
        cx_adt_rest.

    "! <p class="shorttext synchronized">Returns 'X' if this view is an analytics query</p>
...
CLASS zcl_sat_adt_cds_elinfo_reader IMPLEMENTATION.
  ...
  METHOD zif_sat_adt_elinfo_reader~read_element_information.
    TRY.
        ...
        ms_element_info = VALUE #( name       = lo_cds_view->mv_view_name
                                   raw_name   = ls_header-entityname_raw
                                   is_query   = is_analytics_query( lo_cds_view )
                                   amdp_ref   = ls_amdp_ref
                                   properties = VALUE #( package      = ls_tadir_props-devclass
...
***                                                         api_state    = get_api_state( lo_cds_view )
        ...
        io_rest_response->set_body_data( content_handler = zcl_sat_adt_ch_factory=>create_cds_elem_info_res_ch( )
                                         data            = ms_element_info ).
      CATCH zcx_sat_data_read_error INTO DATA(lx_read_error).
        RAISE EXCEPTION TYPE zcx_sat_adt_element_info
          EXPORTING textid = cx_adt_rest=>create_textid_from_exc_text( exception = lx_read_error ).
      CATCH cx_adt_rest INTO DATA(lx_rest).
        RAISE EXCEPTION TYPE zcx_sat_adt_element_info
          EXPORTING textid = cx_adt_rest=>create_textid_from_exc_text( exception = lx_rest ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD get_api_state.
    DATA(lt_api_state_fix_vals) = CAST cl_abap_elemdescr( cl_abap_typedescr=>describe_by_data(
                                                              VALUE zsat_cds_api_state( ) ) )->get_ddic_fixed_values( ).
    DATA(lt_api_states) = io_cds_view->get_api_states( ).

    IF lt_api_states IS INITIAL.
      RETURN.
    ENDIF.
  ...
  METHOD fill_association_infos.
    LOOP AT it_association ASSIGNING FIELD-SYMBOL(<ls_assoc>).
      ...
      <ls_assoc_line>-object   = VALUE #( name        = <ls_assoc>-ref_cds_view
      ...
      DATA(ls_object_ref) = zcl_sat_adt_util=>create_adt_uri( iv_type  = <ls_assoc>-entity_type
  ...
  METHOD fill_base_table_infos.
    LOOP AT it_base_tables ASSIGNING FIELD-SYMBOL(<ls_base_entity>).
      ...
      <ls_select_from> = VALUE #( name        = <ls_base_entity>-entityname
      ...
      DATA(ls_object_ref) = zcl_sat_adt_util=>create_adt_uri( iv_type  = <ls_base_entity>-table_kind
