"! <p class="shorttext synchronized">Provides element information for ADT object for a given uri</p>
...
CLASS zcl_sat_adt_res_eleminfo_byuri IMPLEMENTATION.
  METHOD get_parameters.
    ...
    TRY.
        io_request->get_uri_query_parameter( EXPORTING name      = zif_sat_c_adt_utils=>c_element_info_parameter-uri
                                                       mandatory = abap_true
    ...
    TRY.
        ...
        ELSE.
          RAISE EXCEPTION TYPE zcx_sat_adt_element_info
            EXPORTING textid = zcx_sat_adt_element_info=>object_type_not_supported
      ...
      CATCH cx_adt_uri_mapping INTO DATA(lx_uri_error).
        RAISE EXCEPTION TYPE zcx_sat_adt_element_info
          EXPORTING textid = cx_adt_rest=>create_textid_from_exc_text( exception = lx_uri_error ).
    ENDTRY.
  ENDMETHOD.
ENDCLASS.
