"! <p class="shorttext synchronized" lang="en">Resource for CDS Post Activation</p>
...
CLASS zcl_sat_adt_res_cds_post_actvt IMPLEMENTATION.
  ...
  METHOD post.
    DATA: single_ddlname      TYPE ddlname,
          ddlname_string_list TYPE string,
          ddlname_range       TYPE zif_sat_ty_global=>ty_t_ddlname_range.

    request->get_uri_attribute(
      EXPORTING
        name  = zif_sat_c_adt_utils=>c_cds_post_activation_param-ddlname
      IMPORTING
        value = single_ddlname ).

    request->get_body_data( EXPORTING content_handler = cl_adt_rest_cnt_hdl_factory=>get_instance( )->get_handler_for_plain_text( )
                            IMPORTING data            = ddlname_string_list ).

    IF ( single_ddlname IS INITIAL AND ddlname_string_list IS INITIAL ) OR
        ( single_ddlname IS NOT INITIAL AND ddlname_string_list IS NOT INITIAL ).
