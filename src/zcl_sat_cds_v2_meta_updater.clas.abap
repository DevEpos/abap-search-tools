"! <p class="shorttext synchronized" lang="en">Updates Metadata for CDS Views v2</p>
CLASS zcl_sat_cds_v2_meta_updater DEFINITION
  ...
  PUBLIC SECTION.
    METHODS:
      ...
      "! <p class="shorttext synchronized" lang="en">Updates Meta Index of CDS Views v2</p>
      update_index,
      "! <p class="shorttext synchronized" lang="en">Removes invalid Meta index of CDS Views v2</p>
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ddlname_range    TYPE zif_sat_ty_global=>ty_t_ddlname_range,
      write_debug_info TYPE abap_bool,
      force_update     TYPE abap_bool.
...
CLASS zcl_sat_cds_v2_meta_updater IMPLEMENTATION.
  ...
  METHOD modify_cds_meta_index.
    ...
    LOOP AT ddls_to_update ASSIGNING FIELD-SYMBOL(<ddls>).
      IF force_update = abap_false AND <ddls>-last_generated_on IS NOT INITIAL.
        IF <ddls>-src-as4date < <ddls>-last_generated_on OR
            ( <ddls>-src-as4date = <ddls>-last_generated_on AND
              <ddls>-src-as4time <= <ddls>-last_generated_at ).
  ...
  METHOD store_meta_information.
    ...
    DATA(meta_head) = VALUE zsatcds2mhead(
      ddlname           = ddls-ddlname
