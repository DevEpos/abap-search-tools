...
CLASS zcl_sat_adt_util DEFINITION
  ...
  PUBLIC SECTION.
    "! <p class="shorttext synchronized">Create ADT URI for the given entity</p>
    ...
    "! <p class="shorttext synchronized">Open Object with ADT Tools</p>
    ...
    "! <p class="shorttext synchronized">Retrieve ADT Object URI for the given name/type</p>
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Check if jump is possible for given object</p>
...
CLASS zcl_sat_adt_util IMPLEMENTATION.
  METHOD get_adt_objects_and_names.
    ...
    cl_wb_object=>create_from_transport_key( EXPORTING  p_object    = lv_obj_type
                                                        p_obj_name  = lv_obj_name
                                             RECEIVING  p_wb_object = DATA(lr_wb_object)
                                             EXCEPTIONS OTHERS      = 1 ).
    IF sy-subrc <> 0.
    ...
    cl_wb_request=>create_from_object_ref( EXPORTING  p_wb_object       = lr_wb_object
                                           RECEIVING  p_wb_request      = DATA(lr_wb_request)
    ...
    ELSE.
      ...
      er_adt_objectref = er_adt_uri_mapper->map_wb_object_to_objref( wb_object = lr_wb_object ).

      IF ev_program IS SUPPLIED.
        er_adt_uri_mapper->map_objref_to_include( EXPORTING uri     = er_adt_objectref->ref_data-uri
                                                  IMPORTING program = ev_program
                                                            include = ev_include ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD is_adt_jump_possible.
    cl_wb_request=>create_from_object_ref( EXPORTING  p_wb_object       = ir_wb_object
                                           RECEIVING  p_wb_request      = DATA(lr_wb_request)
                                           EXCEPTIONS illegal_operation = 1
                                                      cancelled         = 2
                                                      OTHERS            = 3 ).
  ...
  METHOD get_adt_object_ref_uri.
    ...
    IF sy-subrc <> 0.
      DATA(ls_obj_ref) = map_tadir_obj_to_object_ref( iv_name = iv_name
                                                      is_type = is_type ).
  ...
  METHOD jump_adt.
    ...
    TRY.
        IF iv_sub_obj_name IS NOT INITIAL.
          ...
          IF    ( lv_program <> iv_obj_name AND lv_include      IS INITIAL )
             OR ( lv_program  = lv_include  AND iv_sub_obj_name IS NOT INITIAL ).
            lv_include = iv_sub_obj_name.
          ENDIF.

          DATA(lr_adt_sub_objref) = lr_adt_uri_mapper->map_include_to_objref( program     = lv_program
                                                                              include     = lv_include
                                                                              line        = iv_line_number
                                                                              line_offset = 0
  ...
  METHOD create_adt_uri.
    ...
    IF    (     iv_name       IS INITIAL
            AND iv_name2      IS INITIAL )
       OR (     iv_type       IS INITIAL
            AND iv_tadir_type IS INITIAL ).
    ...
    IF sy-subrc <> 0.
      ...
      TRY.
          zcl_sat_adt_util=>get_adt_objects_and_names( EXPORTING iv_obj_name      = lv_name
                                                                 iv_obj_type      = lv_obj_type
                                                       IMPORTING er_adt_objectref = DATA(lr_adt_objectref) ).
  ...
  METHOD map_uri_to_wb_object.
    ...
    DATA(lo_wb_object) = lo_adt_tools_core_f->get_uri_mapper( )->map_objref_to_wb_object( uri = iv_uri ).
