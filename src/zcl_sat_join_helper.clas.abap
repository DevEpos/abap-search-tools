"! <p class="shorttext synchronized">Helper for table joins</p>
CLASS zcl_sat_join_helper DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES tt_join_conditions TYPE STANDARD TABLE OF ty_join_condition WITH EMPTY KEY.

    "! <p class="shorttext synchronized">Builds FROM SQL clause for join definition</p>
    ...
    "! <p class="shorttext synchronized">Builds where clause for given conditions</p>
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_join_table TYPE STANDARD TABLE OF ty_join_table WITH EMPTY KEY.
    TYPES:
      BEGIN OF ty_join,
    ...
    "! <p class="shorttext synchronized">Fills missing operators, etc.</p>
    ...
    "! <p class="shorttext synchronized">Prepares join table definitions for building FROM Strings</p>
    ...
    "! <p class="shorttext synchronized">Append parameters string to table clause</p>
...
CLASS zcl_sat_join_helper IMPLEMENTATION.
  ...
  METHOD prepare_tables.
    ...
    IF is_join_def-parameters IS NOT INITIAL.
      append_params_to_table_string( EXPORTING it_parameters = is_join_def-parameters
    ...
    LOOP AT is_join_def-tables ASSIGNING FIELD-SYMBOL(<ls_join_table>) WHERE is_virtual = abap_false.
      ...
      DATA(lv_join_table_alias) = COND #( WHEN <ls_join_table>-add_table_alias IS NOT INITIAL
                                          THEN <ls_join_table>-add_table_alias ).
      ...
      DATA(ls_parsed_table) = VALUE ty_join_table( join_type = lv_join_type_text
                                                   table     = |{ <ls_join_table>-add_table }| ).

      IF <ls_join_table>-parameters IS NOT INITIAL.
        append_params_to_table_string( EXPORTING it_parameters = <ls_join_table>-parameters
      ...
      LOOP AT <ls_join_table>-conditions INTO DATA(ls_cond).
        ...
        ELSE.
          ...
          IF ls_cond-operator = 'BETWEEN'.
            lv_value2 = COND #(
              WHEN ls_cond-value_type = zif_sat_c_join_cond_val_type=>system_value_input
              THEN |@{ ls_cond-value2 }|
              ELSE cl_abap_dyn_prg=>quote( ls_cond-value2 ) ).
  ...
  METHOD repair_join_definition.
    ...
    IF     is_join_def-primary_table_entity_type = zif_sat_c_entity_type=>cds_view
       AND ( if_use_ddl_for_select = abap_true OR sy-saprl < 750 ).
